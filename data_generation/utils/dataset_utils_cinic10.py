...
alpha = 0.1 # for Dirichlet distribution initial 0.1 (original)
...
def separate_data(data, num_clients, num_classes, niid=False, balance=False, partition=None, class_per_client=2):
    ...
    if partition == 'pat':
        ...
        class_num_per_client = [class_per_client for _ in range(num_clients)]
        for i in tqdm(range(num_classes), desc="Distributing Classes"):
            selected_clients = []
            for client in range(num_clients):
                if class_num_per_client[client] > 0:
                    selected_clients.append(client)
            ...
            idx = 0
            for client, num_sample in zip(selected_clients, num_samples):
                dataidx_map[client].extend(idx_for_each_class[i][idx:idx + num_sample])
                idx += num_sample
                class_num_per_client[client] -= 1
        ...
        K = num_classes
        N = len(dataset_label)
        min_samples_per_client = int(least_samples)

        print(f"Distributing data with Dirichlet(alpha={alpha}) over {num_clients} clients")

        for k in tqdm(range(K), desc="Distributing Classes (Dirichlet)"):
            idx_k = np.where(dataset_label == k)[0]
            np.random.shuffle(idx_k)
            ...
            for client_id, idx in enumerate(split_indices):
                dataidx_map[client_id].extend(idx.tolist())

        # Warn if some clients have very few samples
        for client in range(num_clients):
            sample_count = len(dataidx_map[client])
            if sample_count < min_samples_per_client:
                print(f"[Warning] Client {client} only has {sample_count} samples (< {min_samples_per_client})")

        # Optional: Remove clients with too few samples
        # dataidx_map = {k: v for k, v in dataidx_map.items() if len(v) >= min_samples_per_client}
        # print(f"Remaining clients after filtering: {len(dataidx_map)}")

    else:
        raise NotImplementedError(f"Partition type {partition} is not implemented.")
...
def split_data(X, y, train_size=0.8):
    ...
    for i in range(len(y)):
        ...
        if n == 0:
            ...
            # 只有1筆資料，全部給train（或test）以避免空集合
            train_data.append({'x': X[i], 'y': y[i]})
            test_data.append({'x': np.array([]), 'y': np.array([])})
            continue

        # 計算切分索引，確保train與test都至少有1筆
        train_count = max(1, int(n * train_size))
...
def save_file(config_path, train_path, test_path, train_data, test_data, num_clients, 
    ...
    # Save testing data with progress bar
