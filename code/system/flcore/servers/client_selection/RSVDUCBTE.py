...
class EnhancedRSVDUCBThompson():
    ...
    def select_clients(self, epoch, gradients):
        ...
        # calculate the weight based on selected clients' performance history for contribution to the global model
        # ensure the weight sum to 1
        if self.performance_history[selected_clients[0]] == np.zeros(1):
            self.contribution_weights = [1/len(selected_clients)] * len(selected_clients)
        else:
            total_performance_score = 0
            for client in selected_clients:
                total_performance_score += self.performance_history[client]
            
            contribution_weights_list = []
            for client in selected_clients:
                contribution_weights_list.append(self.performance_history[client] / total_performance_score)
        
            self.contribution_weights = contribution_weights_list
