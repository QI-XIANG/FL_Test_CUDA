...
from flcore.servers.client_selection.ReputationGuard import ReputationGuardFL

class FedGuard(Server):
    ...
    def train(self):
        ...
        if self.select_clients_algorithm == "Random":
        ...
        elif self.select_clients_algorithm == "ReputationGuard":
            select_agent = ReputationGuardFL(self.num_clients, self.num_join_clients, self.global_accuracy_history)
            ...
            raise ValueError(f"Unknown client selection algorithm: {self.select_clients_algorithm}")
        ...
        with mlflow.start_run(run_name=f"noniid_wbn_{self.num_clients*self.poisoned_ratio}_same"):
            ...
            for i in range(self.global_rounds + 1):
                ...
                print(f"Poisoned clients among FedGuard clients: {poisoned_selected}")
                ...
                for client in self.selected_clients:
                    ...
                    client.train()
                    if self.select_clients_algorithm in ["ReputationGuard"]:
                ...
                if self.select_clients_algorithm in ["ReputationGuard"] and self.gradients_available and counter_for_RSVD > 1:
                    # Filter gradients based on selected clients
                    selected_gradients = {cid: self.client_gradients[cid] for cid in selected_ids}
                    # Calculate robust rewards based on gradient conformity
                    rewards_dict = select_agent.calculate_robust_rewards(selected_gradients)
                    
                    # Print the calculated robust rewards
                    print(f"Calculated robust rewards: { {k: f'{v:.3f}' for k, v in rewards_dict.items()} }")

                    # Update the agent's reputation model
                    select_agent.update(list(rewards_dict.keys()), list(rewards_dict.values()))
                else: # If gradients are not available
                    pass

                if counter_for_RSVD > 1:
                    # Get aggregation weights based on client reputation
                    aggregation_weights = select_agent.get_aggregation_weights(selected_ids)
                    print(f"Calculated aggregation weights: { {k: f'{v:.3f}' for k, v in aggregation_weights.items()} }")
                    # Create ordered list of weights for the aggregation function
                    ordered_weights = [aggregation_weights[cid] for cid in selected_ids]
                    # Aggregate models using the reputation-based weights
                    self.aggregate_parameters_bn(ordered_weights)
                else:
